...
class CollaborationNetworkAnalyzer:
    ...
    def load_data(self):
        ...
        # Load IMDb cache for consistent cast data (same source as batch 2)
        imdb_cache_dir = self.data_dir / 'processed' / 'imdb_cache'
        self.imdb_names = None
        self.imdb_principals = None
        self.imdb_crew = None
        
        if (imdb_cache_dir / 'names.parquet').exists():
            self.imdb_names = pd.read_parquet(imdb_cache_dir / 'names.parquet')
            print(f"✓ Loaded {len(self.imdb_names):,} names from IMDb cache")
        if (imdb_cache_dir / 'principals.parquet').exists():
            self.imdb_principals = pd.read_parquet(imdb_cache_dir / 'principals.parquet')
            print(f"✓ Loaded {len(self.imdb_principals):,} principals from IMDb cache")
        if (imdb_cache_dir / 'crew.parquet').exists():
            self.imdb_crew = pd.read_parquet(imdb_cache_dir / 'crew.parquet')
            print(f"✓ Loaded {len(self.imdb_crew):,} crew from IMDb cache")

        print()

    def build_collaboration_networks(self):
        """Build collaboration networks from actors_master (same source as batch 2)."""
        print("Building collaboration networks...")

        # Use actors_master.parquet if available (generated by batch 2)
        actors_master_path = self.data_dir / 'processed' / 'actors_master.parquet'
        if actors_master_path.exists():
            self._build_networks_from_actors_master(actors_master_path)
        elif self.imdb_principals is not None and self.imdb_names is not None:
            print("  ⚠️ actors_master.parquet not found, using IMDb cache")
            self._build_networks_from_imdb_cache()
        else:
            print("  ⚠️ IMDb cache not available, falling back to CSV columns")
            self._build_networks_from_csv()

    def _build_networks_from_actors_master(self, actors_master_path):
        """Build networks using actors_master.parquet (canonical source from batch 2)."""
        print("  Using actors_master.parquet for consistent data with batch 2...")
        
        actors_df = pd.read_parquet(actors_master_path)
        print(f"  Loaded {len(actors_df):,} actor-film pairs")
        
        # Build name mapping from actor_id to actor_name
        self.imdb_to_name = dict(zip(actors_df['actor_id'], actors_df['actor_name']))
        
        # Get movie metadata
        movie_info = self.movies_df.set_index('const')[['Title', 'year', 'imdb_rating']].to_dict('index')
        
        # Build collaborations - group by film and count pairs
        ...
        # Deduplicate actors per film first
        actors_per_film = actors_df[['film_id', 'actor_id', 'actor_name']].drop_duplicates()
        
        for film_id, group in actors_per_film.groupby('film_id'):
            actors = group['actor_id'].unique().tolist()
            movie_data = movie_info.get(film_id, {})
            movie_title = movie_data.get('Title', 'Unknown')
            movie_year = movie_data.get('year', '')
            movie_rating = movie_data.get('imdb_rating', 0)
            ...
            if len(actors) >= 2:
                for actor1, actor2 in combinations(actors, 2):
                    actor_collaborations[actor1][actor2].append({
                        'movie': movie_title,
                        'year': movie_year,
                        'rating': movie_rating if pd.notna(movie_rating) else 0
                    })
        
        # Build director-actor collaborations from crew if available
        if self.imdb_crew is not None:
            watched_tconsts = set(self.movies_df['const'].astype(str))
            crew = self.imdb_crew[self.imdb_crew['tconst'].isin(watched_tconsts)].copy()
            
            # Use imdb_names for directors (they're not in actors_df)
            # and actors_df names for actors
            director_names = dict(zip(self.imdb_names['nconst'], self.imdb_names['primaryName'])) if self.imdb_names is not None else {}
            actor_names = self.imdb_to_name
            
            # Add director names to the main lookup for visualization later
            self.imdb_to_name.update(director_names)
            
            for _, row in crew.iterrows():
                tconst = row['tconst']
                movie_data = movie_info.get(tconst, {})
                movie_title = movie_data.get('Title', 'Unknown')
                movie_year = movie_data.get('year', '')
                movie_rating = movie_data.get('imdb_rating', 0)
                ...
                # Get actors for this film from actors_df
                film_actors = actors_per_film[actors_per_film['film_id'] == tconst]['actor_id'].tolist()
                
                for director_id in directors:
                    director_id = str(director_id).strip("[] '\"")
                    # Directors are looked up in full IMDb names, actors in actors_df
                    if director_id in director_names:
        ...
        self._build_graphs(actor_collaborations, director_actor_collaborations)

    def _build_networks_from_imdb_cache(self):
        """Build networks using IMDb cache (same source as batch 2)."""
        print("  Using IMDb cache for consistent data...")
        
        # Get watched film IDs
        watched_tconsts = set(self.movies_df['const'].astype(str))
        
        # Filter principals to watched films and acting roles
        acting_categories = {'actor', 'actress', 'self'}
        principals = self.imdb_principals[
            (self.imdb_principals['tconst'].isin(watched_tconsts)) &
            (self.imdb_principals['category'].isin(acting_categories))
        ].copy()
        
        # Build nconst to name mapping
        names_dict = dict(zip(self.imdb_names['nconst'], self.imdb_names['primaryName']))
        self.imdb_to_name = names_dict
        
        # Get movie metadata for ratings
        movie_info = self.movies_df.set_index('const')[['Title', 'year', 'imdb_rating']].to_dict('index')
        
        # Build actor collaborations by grouping principals by film
        actor_collaborations = defaultdict(lambda: defaultdict(list))
        director_actor_collaborations = defaultdict(lambda: defaultdict(list))
        
        # Group actors by film
        for tconst, group in principals.groupby('tconst'):
            actors = group['nconst'].unique().tolist()
            movie_data = movie_info.get(tconst, {})
            movie_title = movie_data.get('Title', 'Unknown')
            movie_year = movie_data.get('year', '')
            movie_rating = movie_data.get('imdb_rating', 0)
            ...
            if len(actors) >= 2:
                for actor1, actor2 in combinations(actors, 2):
                    if actor1 in names_dict and actor2 in names_dict:
                        actor_collaborations[actor1][actor2].append({
                            'movie': movie_title,
                            'year': movie_year,
                            'rating': movie_rating if pd.notna(movie_rating) else 0
        ...
        # Build director-actor collaborations from crew
        if self.imdb_crew is not None:
            crew = self.imdb_crew[self.imdb_crew['tconst'].isin(watched_tconsts)].copy()
            ...
            for _, row in crew.iterrows():
                ...
                film_actors = principals[principals['tconst'] == tconst]['nconst'].tolist()
                
                for director_id in directors:
                    director_id = str(director_id).strip("[] '\"")
                    if director_id in names_dict:
                        for actor_id in film_actors:
                            if actor_id in names_dict:
                                director_actor_collaborations[director_id][actor_id].append({
                                    'movie': movie_title,
                                    'year': movie_year,
                                    'rating': movie_rating if pd.notna(movie_rating) else 0
    ...
    def _build_networks_from_csv(self):
        """Build networks from CSV columns (fallback)."""
        ...
        for _, movie in self.movies_df.iterrows():
            ...
            # Normalize IDs to strings and strip whitespace, then DEDUPLICATE
            cast_ids = list(set([str(id).strip() for id in cast_ids]))
            director_ids = list(set([str(id).strip() for id in director_ids]))
    ...
    def _build_graphs(self, actor_collaborations, director_actor_collaborations):
        """Build NetworkX graphs from collaboration dictionaries."""
