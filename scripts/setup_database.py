...
This script downloads IMDb's non-commercial datasets and creates a local 
SQLite database. The database will be used to enrich your personal movie data.

Files will be downloaded to: data/raw/
Database will be created at: data/raw/imdb.db
...
def download_imdb_datasets():
    ...
    logger.info(f"Download location: {settings.RAW_DATA_DIR}")
...
class IMDbDatabaseSetup:
    ...
    def create_tables(self):
        ...
        # title.ratings - IMDb ratings and vote counts
        cursor.execute("""
        ...
        # name.basics - Information about people (actors, directors, etc.)
        cursor.execute("""
    ...
    def import_tsv_to_table(self, file_name: str, table_name: str, columns: list, 
                           filter_types: list = None, chunksize: int = 50000):
        """
        ...
        Args:
            ...
            filter_types (list): For title_basics, list of titleTypes to keep.
        ...
        with gzip.open(file_path, 'rt', encoding='utf-8') as f_in:
            ...
            with tqdm(total=total_lines, desc=f"Processing {file_name}") as pbar:
                for chunk in reader:
                    # Filter and clean data based on table
                    if table_name == 'title_basics' and filter_types:
                        chunk = chunk[chunk['titleType'].isin(filter_types)]
    ...
    def create_indexes(self):
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_title_basics_tconst ON title_basics(tconst);")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_title_basics_primaryTitle ON title_basics(primaryTitle);")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_title_basics_titleType ON title_basics(titleType);")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_title_ratings_tconst ON title_ratings(tconst);")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_title_crew_tconst ON title_crew(tconst);")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_name_basics_nconst ON name_basics(nconst);")
...
def main():
    ...
    logger.info(f"\n📊 Creating database at: {settings.IMDB_DATABASE}")
    
    with IMDbDatabaseSetup(settings.IMDB_DATABASE, settings.RAW_DATA_DIR) as db_setup:
            db_setup.create_tables()
            
            # Import title basics FIRST (it's referenced by other tables)
            db_setup.import_tsv_to_table(
                "title.basics.tsv.gz", 
                "title_basics",
                ['tconst', 'titleType', 'primaryTitle', 'originalTitle', 'isAdult', 
                'startYear', 'endYear', 'runtimeMinutes', 'genres'],
                filter_types=['movie', 'tvMovie', 'video', 'tvSeries', 'tvMiniSeries', 
                            'tvShort', 'tvSpecial']
            )
            
            # Import title.akas - Alternative/localized titles
            db_setup.import_tsv_to_table(
                "title.akas.tsv.gz",
                "title_akas",
            ...
            # Import crew (directors and writers)
            db_setup.import_tsv_to_table(
                "title.crew.tsv.gz", 
                "title_crew",
            ...
            # Import episode information (for TV series)
            db_setup.import_tsv_to_table(
                "title.episode.tsv.gz",
                "title_episode",
            ...
            # Import principals (cast and crew) - LARGE FILE, do last
            db_setup.import_tsv_to_table(
                "title.principals.tsv.gz",
                "title_principals",
    ...
    logger.info("="*70)
    logger.info("✅ IMDb Database setup complete!")
    logger.info(f"Database location: {settings.IMDB_DATABASE}")
    logger.info("")
    logger.info("Next steps:")
    logger.info("  1. Run: python scripts/00_create_master_list.py")
    logger.info("  2. Run: python scripts/enrich/01_enrich_tmdb.py")
    logger.info("="*70)
